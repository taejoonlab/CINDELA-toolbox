...
dirname_tview = sys.argv[2]

samples = dict()
f_list = open('SAMPLE_LIST', 'r')
for line in f_list:
    tokens = line.strip().split()
    samples[tokens[0]] = tokens[1]
f_list.close()
...
for line in f_fa:
    if line.startswith('>'):
        ...
        tview_list = dict()
        for tmp_sample in samples.keys():        
            tview_list[tmp_sample] = []

        for tmp_filename in os.listdir(dirname_tview):
            if tmp_filename.startswith(tmp_var_id):
                for tmp_sample in samples.keys():
                    if tmp_filename.find(samples[tmp_sample]) >= 0:

                        tmp_f = open(os.path.join(dirname_tview, tmp_filename), 'r')
                        tview_list[tmp_sample] = tmp_f.readlines()
                        tmp_f.close()
